...
let bcol = document.getElementById('bgcolorpicker');
let col = document.getElementById('colorpicker');

function eraser(){
    currentColor=ctx.fillStyle;
    strokeColor=ctx.fillStyle;
}
...
function setupCanvas(){
    ...
    bcol.addEventListener('input',function(){
...
function getPolygonPoints(){
    ...
    let radius = Math.hypot(loc.x-mousedown.x,loc.y-mousedown.y);
    for(let i = 0; i < polygonSides; i++){
        polygonPoints.push(new PolygonPoint(mousedown.x + radius * Math.sin(angle),
        mousedown.y + radius * Math.cos(angle)));
